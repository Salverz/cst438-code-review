...
vector<string> retrievePosts(const string& username){
    ...
    for(Post post : posts){
        if(post.userId != username){
            if(post.visibility == "public"){
                ...
                const auto& userIt = find_if(users.begin(), users.end(), [&post](const User& u) { return u.username == post.userId; });
                if (userIt != users.end() && find(userIt->friends.begin(), userIt->friends.end(), username) != userIt->friends.end()) {
                    visiblePosts.push_back(post.postId);
                }
            }
        }
...
int main(){
    ...
    while(loop){
        ...
        if(choice == 1){
            ...
            cin >> username;
            cout << checkVisibility(postId, username);
        }else if(choice == 3){
